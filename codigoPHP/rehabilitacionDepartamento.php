...
<html>
    <head>
        <title>Alta de Departamento Tema 4</title>
        <meta charset="UTF-8">
            ...
            if (isset($_REQUEST['codDep']) || isset($_REQUEST['codDepartamento'])) {//si el codigo de departamento a sido enviado por get entra
                ...
                try {
                    ...
                    if (isset($_REQUEST['codDep'])) {
                        $cod = $_REQUEST['codDep'];
                    } else if (isset($_REQUEST['habilitar'])) {
                ...
                if ($existe == 1) {
                    if (isset($_REQUEST['codDepartamento'])) {
                        ...
                        //CONEXION BD PARA habilitar
                        try {
                            $miDB = new PDO(DNS, USER, PASSWORD);
                            $miDB->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                            $sqlhabilitar = "UPDATE Departamento SET FechaBaja=:fechaAlta WHERE CodDepartamento = :CodDepartamento"; //Creamos la consulta mysql con los parametros bind
                            $habilitarDepartamento = $miDB->prepare($sqlhabilitar); //Preparamos la consulta
                            //bindeamos los campos para la consulta
                            $habilitarDepartamento->bindParam(":fechaAlta", $fechaAlta);
                            $habilitarDepartamento->bindParam(":CodDepartamento", $codDep);
                            $habilitarDepartamento->execute(); //Ejecutamos la consulta preparada

                            echo "<div id='menError'><h3>El departamento " . $codDep . " ha sido dado de alta correctamente</h3></div>";
                        } catch (PDOException $miExcepcionPDO) {//declaración de excepcionesPDO
                            echo "<div class='box'>";
                            echo "<p class='error'>Error " . $miExcepcionPDO->getMessage() . "</p>";
                            echo "<p class='error'>Cod.Error " . $miExcepcionPDO->getCode() . "</p>";
                            ...
                            while ($dep = $consultaDepartamento->fetchObject()) {
                                ...
                                <div id="form">
                                    <form class="descript" action= "<?php echo $_SERVER["PHP_SELF"] ?>" method= "POST">
                                        ...
                                        <div class = "botonSend">
                                            <input class = "botonEnvio" type = "submit" value = "Rehabilitar" name = "habilitar">
